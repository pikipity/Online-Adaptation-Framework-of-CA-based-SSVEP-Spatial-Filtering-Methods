classdef online_mscca < handle
    ...
    methods
        function obj = online_mscca(freqs, phases, harmonic_num, fs,...
                                    ch_num, sig_len)
            L = sig_len;
            ...
            obj.Omega_Y = zeros(2*harmonic_num, ch_num);
            obj.Omega_X = zeros(stim_num, sig_len, ch_num);
            obj.F = zeros(ch_num, ch_num);
            obj.K_cov = zeros(ch_num, ch_num);
            obj.combine_matrix_F = [zeros(2*harmonic_num), eye(2*harmonic_num);
                                    eye(2*harmonic_num),   eye(2*harmonic_num)];
            obj.combine_matrix_K = [zeros(sig_len), eye(sig_len);
        ...
        function update_parameters(obj, X_new, predict_ind)
            Q_X_new = obj.get_ref_sig(predict_ind).' * X_new;
            tmp = [obj.Omega_Y; % Omega_Y (k)
                   Q_X_new];
            obj.F = obj.F + tmp.' * obj.combine_matrix_F * tmp; % F (k+1)

            obj.Omega_Y = obj.Omega_Y + Q_X_new; % Omega_Y (k+1)

            tmp = [obj.get_Omega_X(predict_ind); % Omega_X (k)
                   X_new];
            obj.K_cov = obj.K_cov + tmp.' * obj.combine_matrix_K * tmp; % K_cov (k+1)

            obj.Omega_X(predict_ind, :, :) = obj.get_Omega_X(predict_ind) + X_new; % Omega_X (k+1)

            [W_store, ~] = solve_lsframe(obj.F, obj.K_cov);
            obj.W_cca = W_store{end};

            obj.V_cca = obj.Omega_Y * obj.W_cca;
        end
        ...
        function [r1_seq, r2_seq, r3_seq, r4_seq, r5_seq, r6_seq] = eval(obj, X)
            ...
            if isnan(obj.W_cca)
                ...
                for stim_ind = 1:obj.stim_num
                    ...
                    r_tmp = corrcoef(X * obj.W_cca(:,1), (obj.get_gen_Omega_X(stim_ind) + obj.get_Omega_X(stim_ind)) * obj.W_cca(:,1));
                    if isnan(r_tmp(1,2))
                        % r3_seq = zeros(1, obj.stim_num);
                        r_tmp = corrcoef(X * obj.W_cca(:,1), obj.get_gen_Omega_X(stim_ind) * obj.W_cca(:,1));
                        r3_seq(stim_ind) = r_tmp(1,2);
                    else
                        r3_seq(stim_ind) = r_tmp(1,2);
                    end
                    [~,~,r_tmp] = canoncorr(X * obj.W_cca(:,1), obj.get_ref_sig(stim_ind));
