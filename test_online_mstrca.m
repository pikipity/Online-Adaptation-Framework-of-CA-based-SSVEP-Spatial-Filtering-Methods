...
save_path = 'test_online_mstrca_res';
...
for run_ind =1:20
    ...
    %
    for time_length = 1.5:-0.1:0.5
        sig_len = round(time_length*fs);
        check_corr = zeros(14, subj_num, 7, size(exp_seq,1));
        check_r_diff = zeros(14, subj_num, 7, size(exp_seq,1), 3);
        for sub_ind = 1:subj_num
            ...
            for subband_ind = 1:subband_num
                for classifier_ind = 1:14
                    classifier{classifier_ind, subband_ind} = online_mstrca(freqs, phases, harmonic_num, fs,...
                                                    ch_num, sig_len);
                end
            end
            for trial_ind = 1:size(exp_seq,1)
                ...
                for classifier_ind = 1:14
                    ...
                    for subband_ind = 1:subband_num
                        ...
                        [r1_seq, r3_seq, r4_seq, r5_seq, r6_seq] = classifier{classifier_ind, subband_ind}.eval(test_x_subband);
                        r_1 = [r_1;
                               r1_seq];
                        if sum(r3_seq) == 0
                            r_2 = [r_2;
