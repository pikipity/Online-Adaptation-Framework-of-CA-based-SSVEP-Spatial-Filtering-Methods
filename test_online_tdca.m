...
save_path = 'test_online_tdca_res';
fs = 250;
...
for run_ind =1:20
    ...
    %
    for time_length = 1.5:-0.1:0.5 %0.5:0.1:1.5 %1.5:-0.1:0.5
        if exist(fullfile(save_path, sprintf('acc_res_T%d_RUN%d_seed%d.mat', time_length*10, run_ind, seed_num)), 'file')
            continue
        end
        sig_len = round(time_length*fs);
        check_corr = zeros(19, subj_num, 9, size(exp_seq,1));
        % check_r_diff = zeros(19, subj_num, 9, size(exp_seq,1), 3);
        for sub_ind = 1:subj_num
            ...
            if exist(fullfile(save_path, sprintf('S%d_T%d_RUN%d_seed%d_classifier.mat', sub_ind, time_length*10, run_ind, seed_num)), 'file')
                variableInfo = who('-file', fullfile(save_path, sprintf('S%d_T%d_RUN%d_seed%d_classifier.mat', sub_ind, time_length*10, run_ind, seed_num)));
                if ismember('check_corr', variableInfo)
                    load(fullfile(save_path, sprintf('S%d_T%d_RUN%d_seed%d_classifier.mat', sub_ind, time_length*10, run_ind, seed_num)), 'check_corr')
                    continue
                else
                    disp(sprintf('Cannot load S%d_T%d_RUN%d_seed%d_classifier.mat, so redo it!!!', sub_ind, time_length*10, run_ind, seed_num))
                end
            end
            classifier = {};
            for subband_ind = 1:subband_num
                for classifier_ind = 1:19
    %                 classifier{classifier_ind, subband_ind} = online_tdca_modify(freqs, phases, harmonic_num, fs,...
    %                                                                       ch_num, sig_len,...
    %                                                                       n_delay-1);
                    classifier{classifier_ind, subband_ind} = online_tdca(freqs, phases, harmonic_num, fs,...
                                                                          ch_num, sig_len,...
                                                                          n_delay-1, 5,...
                                                                          n_component);
                end
            ...
            for trial_ind = 1:size(exp_seq,1)
                ...
                for classifier_ind = 1:19
                    ...
                    for subband_ind = 1:subband_num
                        ...
                        r_4 = [r_4;
                               sign(r1_seq).*r1_seq.^2 + sign(r3_seq).*r3_seq.^2];
    %                     if sum(r3_seq) == 0 && sum(r4_seq) == 0
    %                         r_4 = [r_4;
    %                                r1_seq];
    %                     else
    %                         r_4 = [r_4;
    %                                sign(r3_seq).*r3_seq.^2 + sign(r4_seq).*r4_seq.^2];
    %                     end
                        r_5 = [r_5;
                               sign(r1_seq).*r1_seq.^2 + sign(r3_seq).*r3_seq.^2 + sign(r4_seq).*r4_seq.^2];
                        r_6 = [r_6;
                               sign(r1_seq).*r1_seq.^2 + sign(r2_seq).*r2_seq.^2 + sign(r3_seq).*r3_seq.^2 + sign(r4_seq).*r4_seq.^2];
    
                        r_7 = [r_7;
                               sign(r1_seq).*r1_seq.^2 + sign(r3_seq).*r3_seq.^2 + sign(r4_seq).*r4_seq.^2 + sign(r5_seq).*r5_seq.^2 + sign(r6_seq).*r6_seq.^2];
                    ...
                    %     r_sort = sort(r_9,'descend');
                    %     check_r_diff(classifier_ind, sub_ind, 9, trial_ind, check_r_diff_ind) = r_sort(1) - r_sort(1+check_r_diff_ind);
                    % end
                    [~,I_1] = max(r_1);
                    [~,I_2] = max(r_2);
                    [~,I_3] = max(r_3);
                    ...
                    if I_9 == exp_seq(trial_ind,1)
                        check_corr(classifier_ind, sub_ind, 9, trial_ind) = 1;
                    end
                    for subband_ind = 1:subband_num
                        test_x_subband = squeeze(test_x(subband_ind, :, :)).';
                        if classifier_ind == 1 
                        ...
                        elseif classifier_ind == 7
                            classifier{classifier_ind, subband_ind}.update_parameters(test_x_subband, I_6)
                        elseif classifier_ind == 8 || classifier_ind == 11 || classifier_ind == 14 || classifier_ind == 17
                            classifier{classifier_ind, subband_ind}.update_parameters(test_x_subband, I_7)
                        elseif classifier_ind == 9 || classifier_ind == 12 || classifier_ind == 15 || classifier_ind == 18
                            classifier{classifier_ind, subband_ind}.update_parameters(test_x_subband, I_8)
                        elseif classifier_ind == 10 || classifier_ind == 13 || classifier_ind == 16 || classifier_ind == 19
                            classifier{classifier_ind, subband_ind}.update_parameters(test_x_subband, I_9)
                        end
                        if classifier_ind<=10
                            classifier{classifier_ind, subband_ind}.update_ir('none', 0.2, [], []) % update_ir('Tikhonov', 0.2, [], []) % 
                        elseif classifier_ind==11 || classifier_ind==12 || classifier_ind==13
                            classifier{classifier_ind, subband_ind}.update_ir('Tikhonov', 0.2, [], []) % update_ir('Tikhonov', 0.2, [], []) % 
                        elseif classifier_ind==14 || classifier_ind==15 || classifier_ind==16
                            classifier{classifier_ind, subband_ind}.update_ir('ridge', 0.2, [], []) % update_ir('Tikhonov', 0.2, [], []) % 
                        elseif classifier_ind==17 || classifier_ind==18 || classifier_ind==19
            ...
            save(fullfile(save_path, sprintf('S%d_T%d_RUN%d_seed%d_classifier.mat', sub_ind, time_length*10, run_ind, seed_num)), 'classifier', 'check_corr', "-v7.3")
        end
        acc = [];
        for sub_ind = 1:subj_num
            for classifier_ind = 1:19
        ...
        save(fullfile(save_path, sprintf('acc_res_T%d_RUN%d_seed%d.mat', time_length*10, run_ind, seed_num)), 'acc', 'check_corr', "-v7.3")
    end
end
warning on